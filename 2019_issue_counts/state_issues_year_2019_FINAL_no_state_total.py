...
digitized_issues_year_state_total['Alaska'] = {}
...
for url in state_urls:
	...
	for issue in title_json['issues']:
		...
		day = issue_date[8:]
		if year in digitized_issues_year_state_total['Alaska']:
			digitized_issues_year_state_total['Alaska'][year] += 1	
		else:
			digitized_issues_year_state_total['Alaska'][year] = 1			
		title_issue_count += 1
...
string_total_items_sorted = sorted(digitized_issues_year_state_total['Alaska'].items())

# OUTPUTS AS CSV FILE
with open('Alaska_total.csv', 'w', newline='') as csv_file:
    writer = csv.writer(csv_file)
    for key, value in string_total_items_sorted:
        writer.writerow(['Alaska', key, value])
